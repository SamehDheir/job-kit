...
export default function ApplicationsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'PENDING' | 'REVIEWED' | 'SHORTLISTED' | 'INTERVIEWING' | 'ACCEPTED' | 'REJECTED' | 'WITHDRAWN'>('all');
 ...
 >Browse Jobs</button>
  ...
  const visibleFilters = ['all', 'PENDING', 'REVIEWED', 'SHORTLISTED', 'INTERVIEWING'] as const;
      ...
      <div className="mx-auto p-8 max-w-6xl">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-8">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              {isActiveHidden ? `${filterLabels[filter]} (${statusCounts[filter]})` : 'More'}
                ...
                {hiddenFilters.map((status) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors ${
                      filter === status ? 'bg-blue-100 text-blue-600 font-medium' : 'text-gray-700'
                    }`}
                  >
                    {filterLabels[status]} ({statusCounts[status]})
                  </button>
